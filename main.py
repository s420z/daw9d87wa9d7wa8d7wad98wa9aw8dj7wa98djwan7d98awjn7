...
CONFIG_DIR = os.path.join(os.environ['LOCALAPPDATA'], 'temp', 'Nebula')
...
class ConfigWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        title_bar.setFixedHeight(50)  # Adjust height as needed
        title_layout = QtWidgets.QHBoxLayout(title_bar)
        title_layout.setContentsMargins(0, 0, 0, 0)  # Remove margins for full width

        # Title Label
        self.title_label = QtWidgets.QLabel(".gg/nebula", self)
        self.title_label.setAlignment(QtCore.Qt.AlignCenter)
        self.title_label.setFont(QtGui.QFont("Arial", 5, QtGui.QFont.Bold))
        self.title_label.setStyleSheet("""
            color: white;
            background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0,
                                        stop:0 #5A189A, stop:0.5 #9D4EDD, stop:1 #C77DFF);
            padding: 10px;
            border-radius: 10px;
        """)

        title_layout.addWidget(self.title_label, alignment=QtCore.Qt.AlignHCenter)  
...
class ESPWindow(QtWidgets.QWidget):
    ...
    def update_scene(self):
        ...
        try:
            ...
            fps_text = self.scene.addText(f"Nebula External JOIN discord.gg/PcSPGrb3qH FOR MORE CHEATS | FPS: {self.fps}", QtGui.QFont('DejaVu Sans', 12, QtGui.QFont.Bold))
            fps_text.setPos(5, 5)
            fps_text.setDefaultTextColor(QtGui.QColor(0, 0, 0))
        except Exception as e:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            pass
    print("Nebula External Successfully Injected!")
    time.sleep(2)
